...
from dataset import BasicDataset, CarvanaDataset
...
def train_model(
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, 'max', patience=5)  # 目标：最大化Dice分数
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                assert images.shape[1] == model.n_channels, \
                    f'网络定义了{model.n_channels}个输入通道，但加载的图像有{images.shape[1]}个通道。请检查图像加载是否正确。'
...
def get_args():
    parser = argparse.ArgumentParser(
        description='在图像和目标掩码上训练Unet')
    parser.add_argument('--epochs', '-e', metavar='E',
                        type=int, default=5, help='训练轮数')
    parser.add_argument('--batch-size', '-b', dest='batch_size',
                        metavar='B', type=int, default=1, help='批次大小')
    parser.add_argument('--learning-rate', '-l', metavar='LR', type=float, default=1e-5,
                        help='学习率', dest='lr')
    parser.add_argument('--load', '-f', type=str,
                        default=False, help='从.pth文件加载模型')
    parser.add_argument('--scale', '-s', type=float,
                        default=0.5, help='图像的缩放因子')
    parser.add_argument('--validation', '-v', dest='val', type=float, default=10.0,
                        help='用作验证集的数据百分比(0-100)')
    parser.add_argument('--amp', action='store_true',
                        default=False, help='使用混合精度')
    parser.add_argument('--bilinear', action='store_true',
                        default=False, help='使用双线性上采样')
...
if __name__ == '__main__':
    ...
    # 根据数据调整参数
    # n_channels=3 适用于RGB图像
    # n_classes是每个像素要获取的概率数
    model = Unet(n_channels=3, n_classes=args.classes, bilinear=args.bilinear)
    model = model.to(memory_format=torch.channels_last)

    logging.info(f'网络:\n'
                 f'\t{model.n_channels} 输入通道\n'
                 f'\t{model.n_classes} 输出通道(类别数)\n'
    ...
    if args.load:
        ...
        logging.info(f'从 {args.load} 加载模型')
    ...
    try:
        ...
        logging.error('检测到OutOfMemoryError!'
                      '启用检查点以减少内存使用量，但这会减慢训练速度。'
                      '考虑启用AMP(--amp)进行快速和内存高效的训练')
